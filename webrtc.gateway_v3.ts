...
export class WebRTCGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  // Nguời dùng tham gia phòng gọi video
  ...
  async handleJoinRoom(
    ...
    try {
      ...
      // Lấy thông tin user
      const finalFullName = currentUser.full_name;
  ...
  @UseGuards(JwtWsAuthGuard)
  @SubscribeMessage('user-toggle-media')
  async handleUserToggleMedia(
    @ConnectedSocket() client: Socket,
    @MessageBody() data: {
      room: string;
    ...
    try {
      // Cập nhật trạng thái media trong DB
      const update: any = {};
      if (data.mediaType === 'video') {
        update['capabilities.video'] = data.enabled;
      } else if (data.mediaType === 'audio') {
        update['capabilities.audio'] = data.enabled;
      }
      ...
      this.server.to(data.room).emit('media-toggled', {
      ...
      this.logger.log(`User ${userId.toString()} toggled ${data.mediaType} to ${data.enabled} in room ${data.room}`);
      return { success: true };
    } catch (error) {
      this.logger.error(`Error in toggle-media: ${error.message}`);
      return { success: false, error: error.message };
    }
  }
}
